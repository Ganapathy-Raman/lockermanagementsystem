...
function ViewMaintenance() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:2026/maintenance/all")
      .then((response) => {
  ...
  const handleDelete = (recordId) => {
    ...
    if (conf) {
      axios
        .delete(`http://localhost:2026/maintenance/${recordId}`)
        .then(() => {
          ...
          {currentRecords.map((d) => (
            <div key={d.recordId} className="bg-white shadow-lg rounded-lg border border-gray-200 p-4">
              <div className="flex flex-col space-y-2">
                <div className="text-lg font-semibold text-gray-800">Maintenance ID: {d.maintenanceId}</div>
                <div className="text-sm text-gray-600">Locker ID: {d.lockerId?.lockerId || 'N/A'}</div>
                <div className="text-sm text-gray-600">Date: {d.maintenanceDate}</div>
                <div className="text-sm text-gray-600">Description: {d.description}</div>
                <div className="text-sm text-gray-600">Cost: {d.maintenanceCost}</div>
              </div>
            </div>
          ))}
        </div>
