...
const Nav = () => {
  ...
  useEffect(() => {
    ...
    const storedNotificationCount = parseInt(sessionStorage.getItem('notificationCount'), 10) || 0;
      ...
      <header className="bg-gray-800 text-white shadow-md p-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center space-x-4">
          <a href="/notification" className="relative flex items-center p-2 text-sm text-white hover:bg-gray-700 rounded-md">
            ...
            {notificationCount > 0 && (
              <span className="absolute top-0 right-0 -mt-1 -mr-1 w-4 h-4 bg-red-600 text-white text-xs rounded-full flex items-center justify-center">{notificationCount}</span>
            )}
          </a>
