...
function ViewFeedback() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:2026/feedback/all")
      .then((response) => {
  ...
  const handleDelete = (recordId) => {
    ...
    if (conf) {
      axios
        .delete(`http://localhost:2026/feedback/${recordId}`)
        .then(() => {
  ...
  const sortedRecords = [...records].sort((a, b) => {
    const idA = typeof a.maintenanceId === 'string' ? parseFloat(a.maintenanceId) : a.maintenanceId;
    const idB = typeof b.maintenanceId === 'string' ? parseFloat(b.maintenanceId) : b.maintenanceId;
    return idB - idA;
  });
          ...
          {currentRecords.map((d) => (
            <div key={d.recordId} className="bg-white shadow-lg rounded-lg border border-gray-200 p-4">
              <div className="flex flex-col space-y-2">
                <div className="text-lg font-semibold text-gray-800">Customer Name: {d.customerId ? d.customerId.customerName : 'N/A'}</div>
                <div className="text-sm text-gray-600">Customer Email: {d.customerId ? d.customerId.email : 'N/A'}</div>
                <div className="text-sm text-gray-600">Gender: {d.customerId ? d.customerId.gender : 'N/A'}</div>
                <div className="text-sm text-gray-600">Date Of Birth: {d.customerId ? d.customerId.dateOfBirth : 'N/A'}</div>
                <div className="text-sm text-gray-600">Phone Number: {d.customerId ? d.customerId.phoneNo : 'N/A'}</div>
                <div className="text-sm text-gray-600">Feedback Date: {d.feedbackDate}</div>
                <div className="text-sm text-gray-600">Rating: {d.rating}</div>
                <div className="text-sm text-gray-600">Additional Comments: {d.comments}</div>
              </div>
            </div>
          ))}
        </div>
